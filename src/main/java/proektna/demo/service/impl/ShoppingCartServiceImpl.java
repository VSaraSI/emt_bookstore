...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public ShoppingCartServiceImpl(ShoppingCartRepository shoppingCartRepository,
                                   ...
                                   BookService bookService, BundleService bundleService) {
    ...
    @Override
    public List<Bundle> listAllBundlesInShoppingCart(Long cartId) {
        if(!this.shoppingCartRepository.findById(cartId).isPresent())
            throw new ShoppingCartNotFoundException(cartId);
        return this.shoppingCartRepository.findById(cartId).get().getBundleList();
    }
    @Override
    ...
    @Override
    public ShoppingCart addBundleToShoppingCart(String username, Long productId) {
        ShoppingCart shoppingCart = this.getActiveShoppingCart(username);
        Bundle bundle = this.bundleService.findById(productId)
        ...
        shoppingCart.getBundleList().add(bundle);
        return this.shoppingCartRepository.save(shoppingCart);
    }
    ...
    @Override
    public void deleteBundleFromShoppingCart(String username, Long bookId) {
        ShoppingCart shoppingCart = this.getActiveShoppingCart(username);
        shoppingCart.getBundleList().removeIf(
                r-> r.getId().equals(bookId));
        this.shoppingCartRepository.save(shoppingCart);
    }
